...
def test_cli_workflows_end_to_end(tmp_path: Path) -> None:
    ...
    assert main(["simulate", "--steps", "8", "--grid-size", "16", "--with-history", "--output", str(simulate_path)]) == 0
    assert simulate_path.exists()

    assert main(["extract", "--steps", "8", "--grid-size", "16", "--output", str(extract_path)]) == 0
    extract_payload = _read_json(extract_path)
    assert "features" in extract_payload
    assert "D_box" in extract_payload["features"]

    assert main(["detect", "--steps", "8", "--grid-size", "16", "--output", str(detect_path)]) == 0
    detect_payload = _read_json(detect_path)
    assert detect_payload["detection"]["anomaly_label"] in {"nominal", "watch", "anomalous"}
    ...
    assert main(["simulate", "--steps", "8", "--grid-size", "16", "--with-history", "--include-arrays", "--output", str(simulate_path)]) == 0
    simulation_payload = _read_json(simulate_path)
    np.save(history, np.array(simulation_payload["history"], dtype=float))

    assert main(["forecast", "--input-npy", str(history), "--horizon", "4", "--output", str(forecast_path)]) == 0
    forecast_payload = _read_json(forecast_path)
    assert forecast_payload["forecast"]["horizon"] == 4

    assert main(["compare", "--steps", "8", "--grid-size", "16", "--output", str(compare_path)]) == 0
    compare_payload = _read_json(compare_path)
    assert "similarity_label" in compare_payload["comparison"]

    assert main(["report", "--steps", "8", "--grid-size", "16", "--horizon", "4", "--output", str(report_manifest_path), "--output-dir", str(report_dir)]) == 0
