...
def test_report_writes_visual_artifacts_and_optional_manifest(tmp_path) -> None:
    ...
    for name in ["report.html", "field.svg", "history_mean.svg", "forecast_final.svg", "comparison_delta.svg", "trajectory.svg", "summary.json"]:
        assert (run_dir / name).exists(), name
    manifest = json.loads((run_dir / 'manifest.json').read_text(encoding='utf-8'))
    assert set(manifest['optional_artifact_list']) >= {"report.html", "field.svg", "trajectory.svg"}
