...
from mycelium_fractal_net.artifact_bundle import sign_artifact, verify_artifact_signature, verify_bundle
...
def test_verify_bundle_valid_tampered_and_missing_cases(tmp_path: Path) -> None:
    ...
    manifest.write_text(json.dumps({'bundle_artifacts': [{'path': 'artifact.txt', 'sha256': __import__('hashlib').sha256(artifact.read_bytes()).hexdigest()}]}, indent=2) + '\n', encoding='utf-8')
    ...
    assert any('sha256-mismatch:artifact.txt' in failure for failure in tampered['manifests'][0]['failures'])
...
def test_symbolic_context_export_is_deterministic_and_compact(tmp_path: Path) -> None:
    seq = simulate_history(__import__('mycelium_fractal_net').SimulationSpec(grid_size=8, steps=6, seed=11))
    ...
    report2 = build_analysis_report(seq, tmp_path / 'repeat', horizon=3, export_symbolic_context=True)
    payload2 = json.loads(((tmp_path / 'repeat' / report2.run_id) / 'symbolic_context.json').read_text(encoding='utf-8'))
...
def test_verify_bundle_cli_returns_nonzero_on_mismatch(tmp_path: Path) -> None:
    ...
    manifest = root / 'release_manifest.json'
    manifest.write_text(json.dumps({'bundle_artifacts': [{'path': 'artifact.txt', 'sha256': '0' * 64}]}, indent=2) + '\n', encoding='utf-8')
