...
def test_neurochem_controls_cover_required_scientific_checks() -> None:
    ...
    proc = subprocess.run([sys.executable, str(root / 'validation' / 'neurochem_controls.py')], cwd=root, capture_output=True, text=True)
    assert proc.returncode == 0, proc.stdout + proc.stderr
    payload = json.loads((root / 'artifacts' / 'evidence' / 'neurochem_controls' / 'neurochem_controls_summary.json').read_text(encoding='utf-8'))
