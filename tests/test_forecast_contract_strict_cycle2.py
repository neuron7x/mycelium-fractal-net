...
from mycelium_fractal_net.types.forecast import ForecastResult, validate_forecast_payload


ROOT = Path(__file__).resolve().parents[1]
BENCHMARK_SPEC = importlib.util.spec_from_file_location('benchmark_quality_module', ROOT / 'benchmarks' / 'benchmark_quality.py')
...
def _valid_payload() -> dict[str, object]:
    return {
        ...
        'benchmark_metrics': {'forecast_structural_error': 0.02, 'adaptive_damping': 0.9},
...
@pytest.mark.parametrize(
    ...
    [
        (('benchmark_metrics', 'forecast_structural_error'), 'forecast_structural_error'),
        (('benchmark_metrics', 'adaptive_damping'), 'adaptive_damping'),
    ],
)
def test_validate_forecast_payload_rejects_missing_required_benchmark_keys(drop_path: tuple[str, str], expected_fragment: str) -> None:
...
@pytest.mark.parametrize('field_name', ['evaluation_metrics', 'benchmark_metrics', 'uncertainty_envelope', 'predicted_state_summary'])
...
def test_benchmark_quality_fails_when_forecast_contract_is_invalid(monkeypatch: pytest.MonkeyPatch) -> None:
    sequence = simulate_scenario('synthetic_morphology')

    class InvalidForecast:
        predicted_states = [[[0.0 for _ in range(sequence.grid_size)] for _ in range(sequence.grid_size)]]
    ...
    monkeypatch.setattr(benchmark_quality, 'forecast_next', lambda *_args, **_kwargs: InvalidForecast())
    with pytest.raises(ValidationError, match='forecast_structural_error'):
        benchmark_quality._persistence_forecast_score(sequence)


def test_benchmark_quality_does_not_backfill_missing_keys_with_zero(monkeypatch: pytest.MonkeyPatch) -> None:
    sequence = simulate_scenario('synthetic_morphology')
