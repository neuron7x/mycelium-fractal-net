...
class TestTDA:
    ...
    def test_vectorizer(self, field):
        from mycelium_fractal_net.analytics.tda import PersistenceTransformer, PersistenceLandscapeVectorizer
    ...
    def test_pipeline(self, field):
        ...
        from mycelium_fractal_net.analytics.tda import PersistenceTransformer, PersistenceLandscapeVectorizer
        pipe = Pipeline([("tda", PersistenceTransformer()), ("vec", PersistenceLandscapeVectorizer())])
...
class TestTransitions:
    def test_trajectory(self, history):
        from mycelium_fractal_net.analytics.topological_transition import wasserstein_persistence_trajectory
        w = wasserstein_persistence_trajectory(history, stride=5)
        assert len(w) > 0 and np.all(w >= 0)

    def test_detect(self, history):
        from mycelium_fractal_net.analytics.topological_transition import detect_topological_transitions
...
class TestInvariantOp:
    ...
    def test_stability_map(self):
        ...
        def gen(v):
            ...
            for t in range(1, 10): h[t] = h[t-1]*0.95
            return h
...
class TestCognitive:
    ...
    def test_invariance_short(self):
        from mycelium_fractal_net.cognitive import invariance_report
        s = FieldSequence(field=np.zeros((8,8)), history=np.zeros((3,8,8)), spec=None, metadata={})
