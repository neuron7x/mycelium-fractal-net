...
from mycelium_fractal_net.core.reaction_diffusion_engine import ReactionDiffusionConfig, ReactionDiffusionEngine
from mycelium_fractal_net.core.simulate import simulate_history
from mycelium_fractal_net.numerics.grid_ops import BoundaryCondition, compute_laplacian, laplacian_backend, njit
...
def test_laplacian_jit_matches_numpy_reference_for_all_boundaries() -> None:
    ...
    for boundary in (BoundaryCondition.PERIODIC, BoundaryCondition.NEUMANN, BoundaryCondition.DIRICHLET):
...
def test_soft_boundary_damping_reduces_hard_clamp_pressure() -> None:
    ...
    without_soft = ReactionDiffusionEngine(ReactionDiffusionConfig(grid_size=6, soft_boundary_damping=0.0, spike_probability=0.0, random_seed=1))
    ...
    with_soft = ReactionDiffusionEngine(ReactionDiffusionConfig(grid_size=6, soft_boundary_damping=0.8, spike_probability=0.0, random_seed=1))
...
def test_memmap_history_roundtrip(tmp_path: Path) -> None:
    seq = simulate_history(SimulationSpec(grid_size=8, steps=6, seed=7), history_backend='memmap', history_dir=tmp_path)
