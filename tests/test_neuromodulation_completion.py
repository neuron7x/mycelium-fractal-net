...
def test_neuromodulation_spec_roundtrip() -> None:
    spec = SimulationSpec(grid_size=16, steps=8, seed=42, neuromodulation=NeuromodulationSpec(
...
def test_plasticity_aware_detect_compare_forecast() -> None:
    ...
    reorganized = simulate_history(SimulationSpec(grid_size=16, steps=8, seed=42, neuromodulation=NeuromodulationSpec(
    ...
    cf = counterfactual(reorganized, SimulationSpec(grid_size=16, steps=8, seed=42, neuromodulation=_gabaa_spec()))
    assert regime.label in {'stable', 'transitional', 'critical', 'reorganized', 'pathological_noise'}
    assert anomaly.regime is not None
    assert 'connectivity_divergence' in result.drift_summary
    assert result.reorganization_label in {'stable', 'transitional', 'critical', 'reorganized', 'pathological_noise'}
...
def test_api_neuromodulation_surface() -> None:
    ...
    response = client.post('/v1/simulate', json={
        ...
        'neuromodulation': {
            ...
            'enabled': True,
            'dt_seconds': 1.0,
            'observation_noise': {'profile': 'observation_noise_bold_like', 'std': 0.0012, 'temporal_smoothing': 0.35},
        },
    })
...
def test_neuromodulation_state_machine_bounds_and_local_offset() -> None:
    seq = simulate_history(SimulationSpec(
        grid_size=16,
        steps=12,
        seed=7,
