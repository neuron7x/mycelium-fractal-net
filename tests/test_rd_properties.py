...
def test_laplacian_of_constant_is_zero(c, N):
    ...
    lap = compute_laplacian(field, boundary=BoundaryCondition.PERIODIC,
...
def test_laplacian_is_linear(alpha, beta, data):
    ...
    N = 16
    u = data.draw(arrays(np.float64, (N, N), elements=st.floats(-10, 10, allow_nan=False, allow_infinity=False)))
    v = data.draw(arrays(np.float64, (N, N), elements=st.floats(-10, 10, allow_nan=False, allow_infinity=False)))

    bc = BoundaryCondition.PERIODIC
    lap_combo = compute_laplacian(alpha * u + beta * v, boundary=bc, check_stability=False)
    lap_separate = alpha * compute_laplacian(u, boundary=bc, check_stability=False) + \
...
def test_laplacian_sum_zero_periodic(data, N):
    ...
    field = data.draw(arrays(np.float64, (N, N),
                             elements=st.floats(-100, 100, allow_nan=False, allow_infinity=False)))
    lap = compute_laplacian(field, boundary=BoundaryCondition.PERIODIC,
                            check_stability=False)
    assert abs(float(lap.sum())) < 1e-8 * N * N, \
        f"Σ Lap ≠ 0: sum = {lap.sum():.2e}"
...
def test_laplacian_self_adjoint_periodic(data, N):
    ...
    u = data.draw(arrays(np.float64, (N, N),
                         elements=st.floats(-10, 10, allow_nan=False, allow_infinity=False)))
    v = data.draw(arrays(np.float64, (N, N),
                         elements=st.floats(-10, 10, allow_nan=False, allow_infinity=False)))
