...
class FeatureVector:
        ...
        if self.values:
            for name in self.feature_names():
                if name in self.values:
                    setattr(self, name, float(self.values[name]))
        else:
    ...
    def __getitem__(self, key: str | int) -> float:
        ...
        if isinstance(key, int):
            return float(self.to_array()[key])
        return self.values[key]

    def __iter__(self):
        """Iterate over feature values in canonical order."""
...
def compute_fractal_features(
    ...
    ----------
    result : SimulationResult
        Simulation result containing field or history.
    ...
    if isinstance(result, SR):
        if config is None:
            config = FeatureConfig()
        field_data: NDArray[np.floating]
        if result.has_history and result.history is not None:
            field_data = result.history
        else:
            field_data = result.field
        return compute_features(field_data, config)

    if isinstance(result, np.ndarray):
        if config is None:
            raise TypeError("Expected SimulationResult, got ndarray.")
        threshold_v = config.threshold_low_mv / 1000.0
        binary = result > threshold_v
        D_box, D_r2 = _box_counting_dimension(
            binary,
            config.min_box_size,
            config.max_box_size,
            config.num_scales,
    ...
    raise TypeError(
        f"Expected SimulationResult or ndarray, got {type(result).__name__}."
    )
...
def compute_box_counting_dimension(
    ...
    *,
    ...
    Wrapper that accepts floating-point fields and applies a threshold (Volts)
    before delegating to the core box-counting routine.
    """
    ...
    D_box, _ = _box_counting_dimension(
        binary, config.min_box_size, config.max_box_size, config.num_scales
    )
    return D_box


class _Stats(dict):
    """Container that supports both mapping access and tuple unpacking."""
...
def compute_basic_stats(
    ...
    """
    Compute basic field statistics and return a dictionary-like container.
    """
    # Convert to mV
...
def compute_features(
    ...
    fv.values = fv.to_dict()
    return fv


def compute_features_from_result(result: "SimulationResult") -> FeatureVector:
    """
    Compute full feature vector from a SimulationResult.

    Falls back to the final field when history is unavailable.
    """
    from mycelium_fractal_net.core.types import SimulationResult as SR
    ...
    if not isinstance(result, SR):
        raise TypeError(
            ...
            "Use compute_features() for raw numpy arrays."
        )
    ...
    return compute_features(field_data)
