...
def run(output_root: str | Path = 'artifacts/scenarios') -> dict[str, str]:
    ...
    np.save(dataset_path, sequence.history if sequence.history is not None else sequence.field[None, :, :])
    expected = {
    ...
    example_cli_path.write_text('mfn forecast --input-npy artifacts/scenarios/regime_transition/regime_transition_history.npy --horizon 8\n', encoding='utf-8')
