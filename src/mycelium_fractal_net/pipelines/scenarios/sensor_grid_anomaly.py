...
def run(output_root: str | Path = 'artifacts/scenarios') -> dict[str, str]:
    ...
    report = build_analysis_report(anomaly, output_root=root / 'runs', horizon=6, comparison_sequence=baseline)
    dataset_path = dataset_dir / 'sensor_grid_history.npy'
    np.save(dataset_path, anomaly.history if anomaly.history is not None else anomaly.field[None, :, :])
    expected = {
    ...
    example_cli_path.write_text('mfn detect --input-npy artifacts/scenarios/sensor_grid_anomaly/sensor_grid_history.npy\n', encoding='utf-8')
