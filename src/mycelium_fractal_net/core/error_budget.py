...
def compute_error_budget(
    ...
    stages.append(StageError(
        stage="simulate", metric="clamping_rate",
        value=clamp_fraction, threshold=0.05, fraction=clamp_fraction / 0.05,
    ))

    # Simulation: occupancy mass error
    occ_error = float(sequence.metadata.get("occupancy_mass_error_max", 0.0))
    stages.append(StageError(
        stage="simulate", metric="occupancy_mass_error",
        value=occ_error, threshold=1e-4, fraction=occ_error / 1e-4 if occ_error > 0 else 0.0,
    ))
    ...
    stages.append(StageError(
        stage="simulate", metric="field_range_utilization",
        value=range_frac, threshold=1.0, fraction=range_frac,
    ))
    ...
    if descriptor is not None:
        ...
        r2_error = 1.0 - d_r2
        stages.append(StageError(
            stage="extract", metric="fractal_regression_error",
            value=r2_error, threshold=0.3, fraction=r2_error / 0.3 if r2_error > 0 else 0.0,
        ))
    ...
    if forecast is not None:
        ...
        stages.append(StageError(
            stage="forecast", metric="structural_error",
            value=se, threshold=0.2, fraction=se / 0.2 if se > 0 else 0.0,
        ))
