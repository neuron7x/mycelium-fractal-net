...
def _spec_from_payload(req: SimulationPayload) -> SimulationSpec:
    ...
    payload['neuromodulation'] = None if neuromod is None else NeuromodulationSpec(
        ...
        gabaa_tonic=None if neuromod.get('gabaa_tonic') is None else GABAATonicSpec(**neuromod['gabaa_tonic']),
        serotonergic=None if neuromod.get('serotonergic') is None else SerotonergicPlasticitySpec(**neuromod['serotonergic']),
        observation_noise=None if neuromod.get('observation_noise') is None else ObservationNoiseSpec(**neuromod['observation_noise']),
    )
...
def build_v1_router() -> APIRouter:
    ...
    async def forecast_endpoint(payload: ForecastPayload) -> dict[str, Any]:
        ...
        if not seq.has_history:
            seq = FieldSequence(field=seq.field, history=seq.field[None, :, :], spec=seq.spec, metadata=seq.metadata)
    ...
    async def compare_endpoint(payload: ComparePayload) -> dict[str, Any]:
        start = time.perf_counter()
        result = compare_sequences(_sequence_from_payload(payload.left), _sequence_from_payload(payload.right)).to_dict()
    ...
    async def report_endpoint(payload: ReportPayload) -> dict[str, Any]:
        ...
        if not seq.has_history:
            seq = FieldSequence(field=seq.field, history=seq.field[None, :, :], spec=seq.spec, metadata=seq.metadata)
        result = build_analysis_report(seq, output_root=payload.output_root, horizon=payload.horizon).to_dict()
...
__all__ = ["API_VERSION", "SimulationPayload", "FieldPayload", "ForecastPayload", "ComparePayload", "ReportPayload", "build_v1_router", "register_canonical_routes", "create_app", "app", "health_payload", "metrics_payload", "v1_router"]
