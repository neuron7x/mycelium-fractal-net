...
def _normalize_triplet(
    ...
    return occ_rest.astype(np.float64), occ_active.astype(np.float64), occ_des.astype(np.float64)
...
def compute_excitability_offset_v(
    ...
    *,
    ...
    occupancy_bias = 0.60 * state.occupancy_active + 0.25 * state.occupancy_resting - 0.15 * state.occupancy_desensitized
    local_offset_mv = float(baseline_activation_offset_mv) + float(rest_offset_mv) * occupancy_bias * (0.50 + 0.50 * excitability_drive)
    if plasticity_scale > 1.0:
        local_offset_mv += float(rest_offset_mv) * 0.10 * (float(plasticity_scale) - 1.0) * state.plasticity_index
...
def step_neuromodulation_state(
    ...
    *,
    ...
    if gabaa:
        ...
        bind_propensity = np.clip(bind_rate * (0.35 * ligand_rest + 0.65 * ligand_active * activity_drive), 0.0, 1.0)
    ...
    plasticity_drive = np.clip(np.abs(activator - np.mean(activator)) * 2.0 * plasticity_scale, 0.0, 1.0)
    if serotonergic:
        plasticity_drive = _clip01(plasticity_drive + float(serotonergic.get('reorganization_drive', 0.0)))
    ...
    if observation_noise:
        target_noise = np.full(shape, max(0.0, float(observation_noise.get('std', 0.0))), dtype=np.float64)
        smoothing = float(np.clip(observation_noise.get('temporal_smoothing', 0.0), 0.0, 1.0))
        observation_noise_gain = state.observation_noise_gain * smoothing + target_noise * (1.0 - smoothing)
    else:
