...
def effective_gabaa_shunt(active_fraction: NDArray[np.float64], shunt_strength: float) -> NDArray[np.float64]:
    return np.clip(active_fraction * max(0.0, shunt_strength), 0.0, 0.95)


def effective_serotonergic_gain(plasticity_drive: NDArray[np.float64], fluidity_coeff: float, coherence_bias: float) -> NDArray[np.float64]:
