...
_REQUIRED_TOP_LEVEL = {
    'profile', 'profile_id', 'evidence_version', 'enabled', 'dt_seconds',
    'intrinsic_field_jitter', 'intrinsic_field_jitter_var',
    'baseline_activation_offset_mv', 'tonic_inhibition_scale', 'gain_fluidity_coeff',
    'gabaa_tonic', 'serotonergic', 'observation_noise',
}
_REQUIRED_GABAA = {
    'profile', 'agonist_concentration_um', 'resting_affinity_um', 'active_affinity_um',
    'desensitization_rate_hz', 'recovery_rate_hz', 'shunt_strength', 'rest_offset_mv',
    'baseline_activation_offset_mv', 'tonic_inhibition_scale', 'k_on', 'k_off', 'K_R',
    'c', 'Q', 'L', 'binding_sites', 'k_leak_reduction_fraction',
}
_REQUIRED_SEROTONERGIC = {
    'profile', 'gain_fluidity_coeff', 'reorganization_drive', 'coherence_bias',
    'plasticity_scale', 'connectivity_flattening_scale', 'complexity_gain_scale',
}
...
def validate_profile_registry(registry: dict[str, dict] | None = None) -> None:
    ...
    for name, profile in registry.items():
        ...
        if profile['serotonergic'] is not None:
            _validate_required(f'{name}.serotonergic', profile['serotonergic'], _REQUIRED_SEROTONERGIC)
        if profile['observation_noise'] is not None:
            _validate_required(f'{name}.observation_noise', profile['observation_noise'], _REQUIRED_OBSERVATION_NOISE)
