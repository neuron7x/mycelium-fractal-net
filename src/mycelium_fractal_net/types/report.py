    ...
    def to_symbolic_context(self, manifest_hashes: dict[str, str] | None = None) -> SymbolicContext:
        ...
        if self.forecast is not None:
            forecast_summary = {
                ...
                'forecast_structural_error': float(self.forecast.benchmark_metrics.get('forecast_structural_error', 0.0)),
                'adaptive_damping': float(self.forecast.benchmark_metrics.get('adaptive_damping', 0.0)),
            }
        compare_summary = {}
        ...
        return SymbolicContext(
            ...
            key_metrics={
                ...
                'field_mean_mV': float(summary.get('field_mean_mV', self.sequence.to_dict().get('field_mean_mV', 0.0))),
                'field_std_mV': float(summary.get('field_std_mV', self.sequence.to_dict().get('field_std_mV', 0.0))),
            },
            regime_labels={
                'anomaly_label': self.detection.label,
                'regime_label': self.detection.regime.label if self.detection.regime else 'n/a',
                'comparison_label': self.comparison.label if self.comparison is not None else 'n/a',
            },
