...
def run_if_std(N: int = 200, T_ms: int = 3000, tau_rec_ms: float = 800.0,
    ...
    from brian2 import (
        ...
        ms, mV, Hz, defaultclock, start_scope, Network,
    )
    ...
    G = NeuronGroup(N, 'dv/dt = (V_rest - v) / tau : volt',
...
def task1_mann_kendall_all() -> dict:
    ...
    print(f"  Neural IF+STD:   tau={mk_neural['tau']:+.3f}  p={mk_neural['p']:.4f}  "
...
def task2_sigma_sweep() -> dict:
    ...
    for tau_rec in tau_recs:
        ...
        print(f"  tau_rec={tau_rec:5d}ms  σ={sigma:.3f}  MK_tau={mk_r['tau']:+.3f}  "
...
def task3_large_network() -> dict:
    ...
    for N in [200, 500]:
        ...
        print(f"  N={N:4d}  σ={sigma:.3f}  MK_tau={mk_r['tau']:+.3f}  "
...
if __name__ == "__main__":
    ...
    print(f"\n  {elapsed:.0f}s total")
    print(f"  Saved: results/neural_M_v2.json")
    print("=" * 60)
