...
def main() -> int:
    ...
    _run([sys.executable, 'scripts/check_openapi_contract.py'], 'check_openapi_contract', 'wave_4')
    _run([sys.executable, 'validation/run_validation_experiments.py'], 'validation_cycle', 'wave_7')
    _run([sys.executable, 'benchmarks/benchmark_core.py'], 'benchmark_core', 'wave_7')
    _run([sys.executable, 'benchmarks/benchmark_scalability.py'], 'benchmark_scalability', 'wave_7')
    _run([sys.executable, 'benchmarks/benchmark_quality.py'], 'benchmark_quality', 'wave_7')
    ...
    for name in ['benchmark_core.json', 'benchmark_core.csv', 'benchmark_scalability.json', 'benchmark_scalability.csv', 'benchmark_quality.json', 'benchmark_quality.csv']:
        src = ROOT / 'benchmarks' / 'results' / name
        if src.exists():
            shutil.copy2(src, RELEASE / 'benchmark_pack' / name)

    val_summary = ROOT / 'artifacts' / 'evidence' / 'wave_7' / 'validation' / 'validation_summary.json'
    ...
    (RELEASE / 'final_release_checklist.json').write_text(json.dumps(checklist, indent=2) + '\n', encoding='utf-8')
