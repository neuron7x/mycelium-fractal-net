...
def main() -> int:
    ...
    payload = {
        ...
        'commands': {
            'python_import': _run([sys.executable, '-c', 'import mycelium_fractal_net; print(mycelium_fractal_net.__version__)']),
            'mfn_help': _run(['mfn', '--help']) if shutil.which('mfn') else {'returncode': 127, 'stderr': 'mfn not found'},
        },
        'env': {
    ...
    (OUT / 'dependency_matrix.json').write_text(json.dumps(payload, indent=2) + '\n', encoding='utf-8')
