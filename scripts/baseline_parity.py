...
def _ensure_showcase_run() -> None:
    if SHOWCASE_MANIFEST.exists():
        return
    subprocess.run([sys.executable, str(ROOT / 'scripts' / 'showcase_run.py')], cwd=ROOT, check=True)
...
def main() -> int:
    ...
    (OUT / 'baseline_parity_report.json').write_text(json.dumps(report, indent=2) + '\n', encoding='utf-8')
