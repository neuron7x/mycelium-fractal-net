...
REQUIRED_PATHS = ['/v1/simulate', '/v1/detect', '/v1/forecast', '/v1/compare', '/v1/report']
...
def _contains_neuromodulation(node: object, schema: dict[str, Any]) -> bool:
    if isinstance(node, dict):
        ...
        return any(k == 'neuromodulation' or _contains_neuromodulation(v, schema) for k, v in node.items())
...
def main() -> int:
    ...
    neuromod_failures = [
        ...
        if path in current.get('paths', {}) and not _contains_neuromodulation(current['paths'][path], current)
    ]
    payload = {
