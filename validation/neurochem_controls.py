...
def _scenario_result(name: str) -> dict:
    ...
    steps = max(1, int(seq.metadata.get('steps_computed', len(seq.history) if seq.history is not None else 1)))
    ...
    return {
        ...
        'fractal_bounds': bool(ACCEPTANCE_THRESHOLDS['fractal_dimension_min'] <= float(desc.complexity.get('fractal_dimension', 0.0)) <= ACCEPTANCE_THRESHOLDS['fractal_dimension_max']),
        'structured_complexity': float(desc.complexity.get('temporal_lzc', 0.0) + 2.0 * desc.neuromodulation.get('plasticity_index', 0.0) + desc.connectivity.get('connectivity_divergence', 0.0)),
        ...
        'occupancy_bounds': bool(seq.metadata.get('occupancy_bounds_ok', occupancy_mass_error <= ACCEPTANCE_THRESHOLDS['occupancy_mass_error_max'])),
...
def run_neurochem_controls(output_root: str | Path = 'artifacts/evidence/neurochem_controls') -> dict:
    ...
    if not (baseline['clamp_events_per_step'] <= ACCEPTANCE_THRESHOLDS['clamp_events_per_step_max']):
        failures.append('baseline_clamp_events_above_threshold')
    if not (gabaa['effective_inhibition'] >= ACCEPTANCE_THRESHOLDS['tonic_inhibition_stability_min']):
    ...
    if not baseline['occupancy_bounds'] or not gabaa['occupancy_bounds'] or not recovery['occupancy_bounds']:
        failures.append('occupancy_bounds_failed')
    if not (max(baseline['occupancy_mass_error_max'], gabaa['occupancy_mass_error_max'], recovery['occupancy_mass_error_max']) <= ACCEPTANCE_THRESHOLDS['occupancy_mass_error_max']):
        failures.append('occupancy_mass_error_above_threshold')
    if not (recovery['desensitization_recovery'] > 0.0 and recovery['occupancy_resting_mean'] > recovery['occupancy_desensitized_mean']):
        failures.append('desensitization_recovery_failed')
    if not (reorg['structured_complexity'] >= baseline['structured_complexity'] + ACCEPTANCE_THRESHOLDS['noise_vs_reorganization_gap_min']):
        failures.append('reorganization_without_structured_complexity_gain')
    if not (reorg['plasticity_index'] >= noise['plasticity_index'] + ACCEPTANCE_THRESHOLDS['noise_vs_reorganization_gap_min']):
    ...
    summary = {
        ...
        'controls': {
            ...
            'occupancy_bounds': all(results[name]['occupancy_bounds'] for name in ('baseline_compatibility', 'gabaa_tonic_stabilization', 'high_inhibition_recovery')),
    ...
    (root / 'neurochem_controls_summary.json').write_text(json.dumps(summary, indent=2) + '\n', encoding='utf-8')
    return summary
